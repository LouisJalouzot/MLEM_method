...
dtw_baseline_dfs = [df.copy() for df in dtw_dfs]

np.random.seed(0)
# Generate a single consistent layer permutation per model to be used across all folds and comparisons
layer_perms = {}
for family, model in index:
    n_layers = len(i_pivot.loc[cvs[0], family, model])
    layer_perms[(family, model)] = np.random.permutation(n_layers)
...
dtw_baseline_sym = [d.combine_first(d.T).fillna(0.0) for d in dtw_baseline_dfs]
...
mlem = MLEM(distance="precomputed", random_seed=0, device="cuda" if torch.cuda.is_available() else "cpu", verbose=True)
...
def load_and_plot_mlem(sym_dfs, output_fi_path):
    fis = []
    for d in tqdm(sym_dfs):
    ...
    ax.set_ylabel("")
    plt.savefig(output_fi_path, bbox_inches="tight")
    plt.close()


load_and_plot_mlem(dtw_sym, "think_alike/figures/dtw_fi.pdf")
load_and_plot_mlem(dtw_baseline_sym, "think_alike/figures/dtw_baseline_fi.pdf")
...
def plot_mds_projection(sym_dfs, output_path):
    ref = fit_mds(sum(sym_dfs) / len(sym_dfs))
    ref = ref - ref.mean(0)

    all_coords = [
        pd.DataFrame(align_to_ref(fit_mds(d), ref), columns=["MDS1", "MDS2"], index=d.index).assign(cv=cv)
        for cv, d in zip(cvs, sym_dfs)
    ]
    summary = (
        pd
        .concat(all_coords)
    ...
    plt.close(fig)
    return all_coords, summary


dtw_sym = [d.combine_first(d.T).fillna(0.0) for d in dtw_dfs]
all_coords, summary = plot_mds_projection(dtw_sym, "think_alike/figures/dtw_mds.pdf")
dtw_baseline_sym = [d.combine_first(d.T).fillna(0.0) for d in dtw_baseline_dfs]
_, _ = plot_mds_projection(dtw_baseline_sym, "think_alike/figures/dtw_baseline_mds.pdf")
