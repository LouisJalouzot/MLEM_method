...
def aggregate_masked_tensor(data: MaskedTensor, dim: int, method: str = "mean") -> torch.Tensor:
    """
    ...
    if method == "mean":
        ...
        raise ValueError(
            f"Unsupported aggregation method: {method}. Choose from 'mean', 'min', 'max', 'first', 'last'."
        )


def aggregate_word_tokens(
    hidden_states: torch.Tensor,
    offsets_mapping: torch.Tensor,
    word_start_index: torch.Tensor,
    word_end_index: torch.Tensor,
    method: str,
) -> torch.Tensor:
    """Aggregate each word's subtoken hidden states.

    Tokens are selected when their tokenizer offsets fall entirely within the
    corresponding word's character span. Special tokens, whose end offset is
    zero, are excluded.

    Args:
        hidden_states: Hidden states shaped
            (n_words, max_seq_len, n_layers+1, hidden_size).
        offsets_mapping: Token character offsets shaped
            (n_words, max_seq_len, 2).
        word_start_index: Word start character offsets shaped (n_words,).
        word_end_index: Word end character offsets shaped (n_words,).
        method: Aggregation method passed to `aggregate_masked_tensor`.

    Returns:
        Aggregated hidden states shaped (n_words, n_layers+1, hidden_size).
    """
    ...
    return aggregate_masked_tensor(data=hidden_states, dim=1, method=method)
...
def compute_hidden_states(
    ...
    token_aggregation: tp.Optional[str] = None,
    char_spans: tp.Optional[tp.Tuple[torch.Tensor, torch.Tensor]] = None,
) -> torch.Tensor | MaskedTensor | tp.Tuple[torch.Tensor | MaskedTensor, ...]:
    """
    ...
    Args:
        ...
        token_aggregation: Aggregate tokens in each inference batch when provided.
        char_spans: Optional per-sentence character start/end tensors. When provided,
            aggregate only tokens contained in each span (for word representations).

    Returns:
        A tensor shaped (n_sentences, n_layers+1, hidden_size) when
        token_aggregation is provided. Otherwise, a MaskedTensor shaped
        (n_sentences, max_seq_len, n_layers+1, hidden_size). Requested offsets
        and input IDs are appended to the return tuple.
    ...
    if char_spans is not None and token_aggregation is None:
        raise ValueError("char_spans requires token_aggregation")
    ...
    encoded_input = tokenizer(
        ...
        return_offsets_mapping=return_offsets_mapping or char_spans is not None,
    )

    # (n_sentences, max_seq_len)
    attention_mask = encoded_input["attention_mask"]
    if return_offsets_mapping or char_spans is not None:
    ...
    with tqdm(
        ...
        for i in range(0, len(sentences), batch_size):
            ...
            batch_hidden_states_permuted = batch_hidden_states.permute(1, 2, 0, 3).cpu()

            if token_aggregation is not None:
                if char_spans is None:
                    token_mask = attention_mask[i : i + batch_size, :, None, None]
                    token_mask = token_mask.bool().broadcast_to(batch_hidden_states_permuted.shape)
                    batch_hidden_states_permuted = aggregate_masked_tensor(
                        masked_tensor(batch_hidden_states_permuted, token_mask),
                        dim=1,
                        method=token_aggregation,
                    )
                else:
                    starts, ends = char_spans
                    batch_hidden_states_permuted = aggregate_word_tokens(
                        hidden_states=batch_hidden_states_permuted,
                        offsets_mapping=offsets_mapping[i : i + batch_size],
                        word_start_index=starts[i : i + batch_size],
                        word_end_index=ends[i : i + batch_size],
                        method=token_aggregation,
                    )

            hidden_states.append(batch_hidden_states_permuted)
    ...
    if token_aggregation is None:
        ...
        attn_mask_4d = attention_mask[:, :, None, None].broadcast_to(hidden_states.shape)
        result = masked_tensor(hidden_states, attn_mask_4d.bool())
    else:
